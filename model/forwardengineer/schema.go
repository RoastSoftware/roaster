// Package forwardengineer was generated automatically by inlinesql at 2018-12-06 11:48:17.919772583 +0100 CET m=+0.001674391.
...
func GetQueries() []string {
	return []string{
		...
		"create table if not exists roast ( id serial not null constraint roast_pk primary key, code text not null constraint code_chk check (char_length(code) <= 500000), username text not null constraint user_fk references \"user\" (username), score numeric(5, 4) not null constraint score_chk check ((score >= (0) :: numeric) AND (score <= (1) :: numeric)), language text not null, create_time timestamp with time zone not null )",
		"create table if not exists warning ( hash uuid not null, row integer not null, \"column\" integer not null, engine text not null, name text not null, description text not null, id serial not null constraint warning_pk primary key )",
		"create unique index if not exists warning_hash_idx on warning (hash)",
		"create table if not exists error ( hash uuid not null, row integer not null, \"column\" integer not null, engine text not null, name text not null, description text not null, id serial not null constraint error_pk primary key )",
