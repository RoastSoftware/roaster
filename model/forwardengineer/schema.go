// Package forwardengineer was generated automatically by inlinesql at 2019-01-05 22:59:18.6727047 +0100 CET m=+0.007774301.
...
func GetQueries() []string {
	return []string{
		...
		"create table if not exists user_followees ( username text not null, create_time timestamp with time zone not null, followee text not null, constraint followee_relation_uq unique (username, followee), constraint username_fk foreign key (username) references \"user\" (username) match simple on update cascade on delete cascade, constraint followee_fk foreign key (username) references \"user\" (username) match simple on update cascade on delete cascade )",
