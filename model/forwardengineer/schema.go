// Package forwardengineer was generated automatically by inlinesql at 2019-01-08 15:12:55.830329896 +0100 CET m=+0.001842869.
...
func GetQueries() []string {
	return []string{
		...
		"create table if not exists \"user\" ( username username not null constraint user_pkey primary key, hash bytea not null, create_time timestamp with time zone not null, fullname fullname, email email not null )",
		...
		"create table if not exists avatar ( avatar bytea not null, username username not null, constraint username_uq unique (username), constraint username_fk foreign key (username) references \"user\" (username) match simple on update cascade on delete cascade )",
